...
BGW_PATH=/global/homes/z/zfzheng/src/bgw-keroutput/bin

EXE=$BGW_PATH/absorption.cplx.x
INFILE='absorption.inp'
OUTFILE='absorption.out'
...
do
    ...
    mkdir -p $PATHQ
    cp -a prep-bse-q/absorption/* $PATHQ/
 
    cd $PATHQ

    sed -i "s/_Q1_/$Q1/g" absorption.inp
    sed -i "s/_Q2_/$Q2/g" absorption.inp
    sed -i "s/_Q3_/$Q3/g" absorption.inp

    if [ $iq -eq 1 ]; then
        sed -i "s/_comm_tag_/# /g" absorption.inp
    else
        sed -i "s/_comm_tag_//g" absorption.inp
    fi
    ...
    if grep 'TOTAL' absorption.out >& /dev/null; then
